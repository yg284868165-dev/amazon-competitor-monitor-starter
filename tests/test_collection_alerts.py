...
def test_recovered_scheduled_failure_does_not_notify(monkeypatch, tmp_path):
    ...
    assert alerts.queue_scheduled_collection_alerts(db, [root]) == 0
    assert alerts.flush_pending_collection_alerts(db) == {"sent": 0, "pending": 0}
    assert sent == []
    assert db.fetchall("SELECT * FROM collection_alerts") == []
    assert db.fetchall("SELECT * FROM notification_logs") == []
